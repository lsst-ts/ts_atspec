...
class TestATSpecCSC(asynctest.TestCase):
        ...
        * exitControl: STANDBY, FAULT to OFFLINE (quit)
        ...
        async with Harness(simulation_mode=1) as harness:
            ...
            if hasattr(harness.remote, "evt_settingsAppliedValues"):
                await harness.remote.evt_settingsAppliedValues.next(
                    flush=False, timeout=BASE_TIMEOUT
                )

            elif hasattr(harness.remote, "evt_settingsApplied"):
                await harness.remote.evt_settingsApplied.next(
                    flush=False, timeout=BASE_TIMEOUT
                )
            ...
            try:
                ...
                await cmd_attr.start(cmd_attr.DataType(), timeout=120.0)
            finally:
                state = await harness.remote.evt_summaryState.next(
                    flush=False, timeout=BASE_TIMEOUT
                )
            ...
            await harness.remote.evt_reportedLinearStagePosition.aget(
                timeout=BASE_TIMEOUT
            )
            await harness.remote.evt_reportedFilterPosition.aget(timeout=BASE_TIMEOUT)
            await harness.remote.evt_reportedDisperserPosition.aget(
                timeout=BASE_TIMEOUT
            )
    ...
    async def test_changeFilter(self):
        ...
        async with Harness(simulation_mode=1) as harness:
            ...
            if hasattr(harness.remote, "evt_settingsAppliedValues"):
                ...
                set_applied = await harness.remote.evt_settingsApplied.next(
                    flush=False, timeout=BASE_TIMEOUT
                )

            else:
                print(
                    "No evt_settingsApplied or evt_settingsAppliedValues published in test_changeFilter"
                )
        ...
        async with Harness(simulation_mode=1) as harness:
            ...
            for ls_pos in [0.0, 100, 500, 900, 1000]:
                with self.subTest(ls_pos=ls_pos):
                    ...
                    await harness.remote.cmd_moveLinearStage.set_start(
    ...
    async def test_homeLinearStage(self):
        ...
        async with Harness(simulation_mode=1) as harness:
            ...
            lpos = await harness.remote.evt_reportedLinearStagePosition.aget(
                timeout=BASE_TIMEOUT
            )
            self.assertFalse(inpos1.inPosition)
            self.assertTrue(inpos2.inPosition)
